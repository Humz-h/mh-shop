...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group transition-transform duration-300 hover:-translate-y-1">
      <div className="relative overflow-hidden rounded-xl bg-gray-1 min-h-[403px] shadow-card hover:shadow-card-hover transition-all duration-300">
        ...
        <div className="flex justify-center items-center overflow-hidden">
          <Image src={getImageUrl(item.imgs?.previews?.[0] || item.imageUrl) || "/images/products/default.svg"} alt={item.title || item.name || ""} width={280} height={280} className="pointer-events-none transition-transform duration-500 ease-smooth group-hover:scale-105" />
        </div>

        <div className="absolute right-0 bottom-0 translate-x-full u-w-full flex flex-col gap-2 p-5.5 ease-linear duration-300 group-hover:translate-x-0 z-10">
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
            ...
            id="bestOne"
            className="flex items-center justify-center w-9 h-9 rounded-lg shadow-1 transition-all duration-200 text-dark bg-white hover:text-white hover:bg-blue-600 cursor-pointer relative z-20"
            type="button"
          >
            <svg
