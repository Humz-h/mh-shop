...
export function FavoriteButton({
  ...
  product,
  className = "",
}: Omit<FavoriteButtonProps, "variant">) {
  const { isAuthenticated } = useAuth();
  const { isFavorite: apiFavorite, toggle, loading } = useFavorite(productId);
  const wishlistItems = useSelector((state: { wishlistReducer: { items: { id: number }[] } }) => state.wishlistReducer.items);
  const inLocalWishlist = wishlistItems.some((i) => i.id === productId);
  const isFavorite = isAuthenticated ? apiFavorite : inLocalWishlist;
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    if (isAuthenticated) {
      ...
      if (!success) {
        window.alert("Không thể cập nhật yêu thích. Vui lòng thử lại sau.");
      }
    } else {
      if (isFavorite) {
  ...
  const baseClass =
    "flex items-center justify-center w-9 h-9 rounded-lg shadow-1 transition-all duration-200 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed";
  const iconClass = isFavorite
    ? "text-red-500 bg-white hover:text-red-600 hover:bg-red-50"
